...
public class TestReinforce {
     ...
     */
    @Test
    public void testStartupReinforce(){
    	int armies_deployed=player1.getAssignedCountries().get(0).getCurrentArmiesDeployed();
    	int armies_reinforce=player1.getReinforceArmies();
    	player1.playerReinforces(armies_reinforce, elements, player1.getAssignedCountries().get(0).getCountryName(), game_view);
    	assertEquals(armies_deployed+armies_reinforce, player1.getAssignedCountries().get(0).getCurrentArmiesDeployed());    	
    }
